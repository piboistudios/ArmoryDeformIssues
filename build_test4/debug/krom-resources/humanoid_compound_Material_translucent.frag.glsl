...
void main()
{
    ...
    vec3 basecol = (((vec3(0.800000011920928955078125) * MixShader_fac_inv) + (Mix_002_Color_res * MixShader_fac)) + Mix_003_Color_res) + vec3(0.800000011920928955078125);
    ...
    float specular = (((((1.0 * MixShader_fac_inv) + (0.0 * MixShader_fac)) * 0.5) + 0.5) * 0.5) + 0.5;
    ...
    if (receiveShadow)
    {
        if (lightShadow == 1)
        {
            ...
            if (lightType == 0)
            {
                int param;
                int param_1;
                mat4 _863 = getCascadeMat(distance(eye, wposition), param, param_1);
                int casi = param;
        ...
        else
        {
            ...
            {
                vec3 param_2 = -l;
                float _922 = PCFCube(shadowMapCube, lp, param_2, shadowsBias, lightProj, n);
                visibility *= _922;
            }
        }
    }
